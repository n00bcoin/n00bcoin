...
unsigned int nStakeMinAge = 60 * 60 * 24 * 15; // 15 days
unsigned int nStakeMaxAge = 60 * 60 * 24 * 30; // 30 days
unsigned int nStakeTargetSpacing = 60 * 60 * 3; //3 minutes
int64 nChainStartTime = 1403413093;
int nCoinbaseMaturity = 99; //99 blocks
...
const string strMessageMagic = "n00bcoin i5 l33t, u R naht :\n";
...
// n00bcoin: increasing Nfactor gradually
...
{
	static const int64 nMin = 0.99999999 * COIN;
	int64 nSubsidy = 99.99999999 * COIN;
	nSubsidy >>= (nHeight / 64000);
	if (nHeight == 1) { nSubsidy = 99999 * COIN; }
	if (nSubsidy < nMin){ nSubsidy = nMin; }
    return nSubsidy;
}

int64 GetProofOfStakeReward(int64 nCoinAge)
{
    static int64 nRewardCoinYear = 99 * CENT;  // creation amount per coin-year
...
static const int64 nTargetSpacingWorkMax = 1 * nStakeTargetSpacing; // 3 min
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "n00bcoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "n00bcoin is caring. n00bcoin is kind. n00bcoin is noob. n00bcoin is moon.";
        CTransaction txNew;
        ...
        assert(block.hashMerkleRoot == uint256("0x074e1478d90e3f1ef2374d619e5ec8f66671ca007d2d06bfbbde81d530dacac2"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xe7, 0xf6, 0xd9 , 0xd5 };
